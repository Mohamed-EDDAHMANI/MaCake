...
export class GatewayForwardService {
  ...
  async getServiceInstance(serviceKey: string): Promise<{
    ...
    // endpointsRaw is stored as an array of { pattern: string, roles: string[] }
    const rawEndpoints = Array.isArray(endpointsRaw) ? endpointsRaw : [];
    const endpoints: { pattern: RegExp; roles: string[] }[] = rawEndpoints.map((entry: any) => ({
  ...
  /**
   * Handle refresh token cookie (supports both top-level and data.refreshToken)
   */
  handleRefreshToken(response: any, res: express.Response): any {
    const token = response?.refreshToken ?? response?.data?.refreshToken;
    if (!token) {
    ...
    const { refreshToken: _rt, ...rest } = response;
    if (response?.data && typeof response.data === 'object' && 'refreshToken' in response.data) {
      const { refreshToken: _drt, ...dataRest } = response.data;
