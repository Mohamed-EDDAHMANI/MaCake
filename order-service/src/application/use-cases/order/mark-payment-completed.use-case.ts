...
export class MarkPaymentCompletedUseCase {
  private readonly logger = new Logger(MarkPaymentCompletedUseCase.name);
  ...
  async execute(orderId: string, clientId: string) {
    ...
    if (!clientId) {
      return new ServiceError('VALIDATION_ERROR', 'Missing client id', 400, 'order-service').toJSON();
    }
    ...
    const updated = await this.orderRepository.update(orderId, { status: OrderStatus.PREPARING });
    if (!updated) {
      return { success: false, message: 'Order not found', data: null };
    }

    this.emitOrderStatusChanged(orderId, OrderStatus.PREPARING);
