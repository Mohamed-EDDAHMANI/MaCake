...
export class StartDeliveryUseCase {
  private readonly logger = new Logger(StartDeliveryUseCase.name);
  ...
  async execute(orderId: string, clientId: string) {
    ...
    const updated = await this.orderRepository.update(orderId, { status: OrderStatus.DELIVERING });
    if (!updated) {
      return { success: false, message: 'Order not found', data: null };
    }

    this.emitOrderStatusChanged(orderId, OrderStatus.DELIVERING);
